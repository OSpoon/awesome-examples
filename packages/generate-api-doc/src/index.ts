      ...
      ExportNamedDeclaration(path, state) {
        ...
        path.node.leadingComments.forEach(comment => {
          if (!comment.skip) {
            ...
            _comment = {
              describe: tags?.find(v => v.title === "cDescribe")?.description || '',
              type: tags?.find(v => v.title === "cType")?.description || '',
              options: tags?.find(v => v.title === "cOptions")?.description || '',
              default: tags?.find(v => v.title === "cDefault")?.description || '',
            };
            comment.skip = true;
          }
        });
        apidoc.push({
          attributeName: path.node.declaration.id.name.substr(1).toLocaleLowerCase(),
